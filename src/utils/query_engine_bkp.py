...
def initialize_engine():
    ...
    # 1) Load the FAISS-backed vector store (binary + metadata)
    vector_store = FaissVectorStore.from_persist_dir(persist_dir=str(STORE_DIR))

    # 2) Load the rest of the storage (docstore, index_store, etc.)
    storage_context = StorageContext.from_defaults(vector_store=vector_store)

    # 3) Reconstruct the VectorStoreIndex
...
#         persist_dir=str(INDEX_PATH / "faiss_store"))
